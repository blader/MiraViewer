...
function buildAcpAnalysisPrompt(opts: {
  ...
  return (
    ...
    + 'The provided image is a capture of the viewer viewport (it may already include zoom/rotation/pan, brightness/contrast adjustments, and cropping to what is visible in the cell).\n\n'
    ...
    + '- segmentation_guide: step-by-step segmentation and annotation guidance. Make it HIGHLY SPECIFIC and LOCALIZING: use concrete anatomical landmarks (sella turcica/pituitary fossa, pituitary stalk, optic chiasm, third ventricle, midline). State where in the image to search (e.g., center/inferior midline vs superior midline), expected shapes, and what to trace first. Include how to distinguish cystic vs solid components and how to mark calcification/hemorrhage cues when visible. Also include guidance for assessing/marking involvement of critical structures (e.g., stalk/chiasm/hypothalamus displacement or compression).\n'
    ...
    + '(6) use separate outlines/contours for each element/component (do not merge into one outline) and use DISTINCT COLORS per element (e.g., tumor boundary vs cystic component vs solid nodule vs calcification markers vs critical structures); '
