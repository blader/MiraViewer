export const clinicalTooltips: Record<string, string> = {
  // T1-weighted
  't1-gre': `T1 GRE (Gradient Echo)
...
• Check third ventricle size - compression causes hydrocephalus`,
  't2-ssfse': `T2 SSFSE (Fast Spin Echo)
...
• Look for tract recovery or worsening over time`,
  'asl': `ASL (Arterial Spin Labeling)
...
• Useful for monitoring without contrast`,
  ...
  // Susceptibility / GRE
  'swi': `SWI/SWAN (Susceptibility-Weighted)
...
export function lookupClinicalTooltip(weight: string | null, sequence: string | null): string | undefined {
  const keyParts = [];
  if (weight) keyParts.push(weight.toLowerCase());
  if (sequence) keyParts.push(sequence.toLowerCase());

  // Try weight+sequence key first.
  const combined = keyParts.join('-');
  if (combined && clinicalTooltips[combined]) return clinicalTooltips[combined];

  // Try sequence-only key (e.g., swi, dwi).
  if (sequence && clinicalTooltips[sequence.toLowerCase()]) return clinicalTooltips[sequence.toLowerCase()];

  // Try weight-only key (e.g., t1, t2).
  if (weight && clinicalTooltips[weight.toLowerCase()]) return clinicalTooltips[weight.toLowerCase()];

  // GRE special case when weight is not T1.
  if (sequence && sequence.toLowerCase() === 'gre' && (!weight || weight.toLowerCase() !== 't1')) {
    return clinicalTooltips['gre-non-t1'];
  }
