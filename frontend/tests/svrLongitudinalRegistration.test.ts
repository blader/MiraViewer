...
describe('svr/longitudinalRegistration', () => {
  it('reslices an 18-degree target stack along the exact reference plane', async () => {
    ...
    if (!result.ok) return;
    const estimate = await estimateLongitudinalRegistration({
    ...
    expect(estimate).toEqual(Object.fromEntries(Object.entries(result).filter(([key]) => !imageFields.has(key))));
    expect(estimate).not.toHaveProperty('pixels');
    expect(estimate).not.toHaveProperty('valid');
    const replay = resliceStackToReferencePlane({
      targetSlices: target,
      referenceSlice: reference[9]!,
      targetToReference: estimate.targetToReference,
      centerMm: estimate.centerMm,
    });
    expect(replay.pixels).toEqual(result.pixels);
    expect(replay.valid).toEqual(result.valid);
