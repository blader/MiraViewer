...
No original MRI, private weights, marks, source, screenshots, prior receipts or comparison bundle was deleted. Diagnostic `[TRACE:...]` instrumentation was confined to a separate ignored build transform; it is absent from the production sources and committed performance test. All eleven normal-production workflows passed on the combined candidate in 43.8 seconds; the current startup/codec check passed in 7.8 seconds. Full lint and both TypeScript checks passed. The first complete unit run passed 3,258 tests and failed one obsolete synchronous-dialog expectation; its focused repair passes without weakening the retained-import assertion. Final-head CI remains required for full-suite clearance. Hosted current-head CI, final review clearance and merge remain distinct from local runtime acceptance.
