...
[Run 33892490513](https://github.com/blader/MiraViewer/actions/runs/33892490513) subsequently passed the new rendering regression, but stopped before browser execution: one deferred-dialog unit test observed the loaded DOM before its focus effect ran. The other 3,259 tests passed. The repair awaits the same focus assertion with the existing default deadline, retaining inert-state, Escape and trigger-restoration checks. No application code, timeout or browser fixture changes; the compiled-app fingerprint and completed production replays remain valid. All six focused dialog tests and the complete local suite then pass: **3,260 passed, zero failed, 54 existing optional skips**, with unchanged source during the 80-second full run.
