...
| `npm run build:browser -- --production`                       | Build the normal production application, without probes, into the same isolated acceptance directory; verify source and copied model assets.   |
...
The repository's [frontend acceptance workflow](../../.github/workflows/frontend.yml) defines a serial hosted-runner gate for lint, the complete default unit suite, production type/build checks and the synthetic normal-app browser workflows. It checks out pinned public model assets through Git LFS, uses read-only repository permissions, and uploads source/model identity plus JSON results and synthetic captures. It does not load private fixtures or alter branch protection. The production build option is for `test:browser`; GPU, performance and pinned-inference probes still require the default `build:browser` mode. A configured workflow is not a passing release gate: inspect the exact commit's completed run and receipts.

The workflow project selects [regular Chromium](https://playwright.dev/docs/browsers#chromium-new-headless-mode), not Playwright's separate headless shell. Local runs default to headless; hosted Linux runs use Xvfb, headed Chromium and Mesa OpenGL. Only this isolated CI project admits the software driver with `--ignore-gpu-blocklist`; Chromium's default Linux software-driver block otherwise rejects WebGL2. The workflow records the actual browser, mode and WebGL renderer in `browser-environment.json`. Browser mode and graphics backend are part of the evidence: a successful local Metal run does not prove that a hosted software renderer passes, and synthetic workflow checks do not establish hardware rendering performance.
...
Development, Vite preview, the offline launcher and the tracked Vercel configuration use `Cross-Origin-Opener-Policy: same-origin` and `Cross-Origin-Embedder-Policy: require-corp`. The offline launcher serves a stable `http://127.0.0.1:43125/` origin with the required WASM MIME types. Keep that origin stable to retain access to saved scans. Vercel builds `frontend/dist`; repository header changes apply only to deployments containing them. Git LFS must be enabled in the project's Git settings (enabled and read back for MiraViewer on September 3). See [Vercel's Git LFS settings](https://vercel.com/docs/project-configuration/git-settings). Runtime assets are served from the application origin, including ORT/WASM, ITK pipelines and pinned model files. Custom hosts must serve those assets with correct MIME types and the same isolation headers to enable multithreaded WASM; without isolation, the runtime deliberately uses one WASM thread. Check the actual origin's `crossOriginIsolated` value, not just its configuration file.

Cornerstone core and the WADO loader initialize once, with decoder workers started only on demand. Interaction handling belongs to the application, not the removed Cornerstone tools/math/Hammer stack. Import, export, help and clear dialogs load when opened. The existing active-dialog state continues to block background interaction while delivery is pending or failed, and the shared accessible dialog preserves close, focus isolation and recovery. No model or offline codec assets are removed to reduce startup bytes.

Warm native reslicing retains at most one successful idle worker for 30 seconds. It is a runtime cache, not a source-slab cache: each request prepares and transfers fresh native inputs. Cancellation, failure and timeout terminate active work; registration-cache invalidation and unmount dispose the idle worker. Automatic slice playback accumulates elapsed steps while required presented targets are computing, then submits one current position. Existing current-request results decide readiness; offscreen work and terminal failures do not block a completed visible pair. Manual navigation and Pause remain available. Native resolution, support and stale-result fences are unchanged. The [paired playback and delivery report](../evidence/startup-delivery-2026-09-04/README.md) records useful completed planes, canceled transfers, exact output parity and the measured memory tradeoff rather than promising a universal frame rate.
...
Exact source-grid recovery is implemented; unverifiable or competing legacy grids remain preserved without automatic assignment. The repository-owned CI gate passed and merged in PR #15. Warm native rendering and local startup/runtime delivery now have measured acceptance; candidate Vercel deployment and current-head merge clearance remain separate gates. Sparse edit persistence and the source-owned settings/acquisition picker have their own linked regression and browser evidence. The implementation ledger is the status authority; old plans and old test totals are historical evidence.
